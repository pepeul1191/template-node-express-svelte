<script>
  ...
  const TABLE_CONFIG = {
    ...
    messages: {
      ...
      errorNetwork: 'No se pudo listar los niveles académicos. No hay conexión con el servidor.',
      notFound: 'No se pudo listar los niveles académicos. Recurso no encontrado.',
      serverError: 'No se pudo listar los niveles académicos. Error interno del servidor.',
      requestError: 'No se pudo listar los niveles académicos. No se recibió respuesta del servidor.',
      otherError: 'No se pudo listar los niveles académicos. Ocurrió un error inesperado.'
    }
  };
  ...
  const deleteLevel = (record) => {
    levelDataTable.deleteRow(record, 'id');
  };

  // Inicialización
  onMount(() => {
    levelDataTable.list();
    levelDataTable.addButton.action = () => levelDataTable.addRow();
    levelDataTable.actionButtons = createActions();
  });
</script>
...
<div class="container-fluid">
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <DataTable
        bind:this={levelDataTable}
        fetchURL={`${API_URL}api/v1/levels`}
        saveURL={`${API_URL}api/v1/levels`}
