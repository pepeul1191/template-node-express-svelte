<script>
  ...
  const TABLE_CONFIG = {
    ...
    messages: {
      ...
      errorNetwork: 'No se pudo listar los tipos de evaluaciones. No hay conexión con el servidor.',
      notFound: 'No se pudo listar los tipos de evaluaciones. Recurso no encontrado.',
      serverError: 'No se pudo listar los tipos de evaluaciones. Error interno del servidor.',
      requestError: 'No se pudo listar los tipos de evaluaciones. No se recibió respuesta del servidor.',
      otherError: 'No se pudo listar los tipos de evaluaciones. Ocurrió un error inesperado.'
    }
  };
  ...
  const deleteLevel = (record) => {
    evaluationTypeDataTable.deleteRow(record, 'id');
  };

  // Inicialización
  onMount(() => {
    evaluationTypeDataTable.list();
    evaluationTypeDataTable.addButton.action = () => evaluationTypeDataTable.addRow();
    evaluationTypeDataTable.actionButtons = createActions();
  });
</script>
...
<div class="container-fluid">
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <DataTable
        bind:this={evaluationTypeDataTable}
        fetchURL={`${API_URL}api/v1/evaluation-types`}
        saveURL={`${API_URL}api/v1/evaluation-types`}
